...
enum intel_port_id_priv {
	PRIV_PORT_ID_NONE = 0,
	PRIV_PORT_A, PRIV_PORT_B, PRIV_PORT_C, PRIV_PORT_D, PRIV_PORT_E, PRIV_PORT_F, PRIV_PORT_G,
	PRIV_MAX_PORTS
};
enum intel_output_type_priv {
	PRIV_OUTPUT_NONE = 0, PRIV_OUTPUT_ANALOG, PRIV_OUTPUT_LVDS, PRIV_OUTPUT_TMDS_DVI,
	PRIV_OUTPUT_TMDS_HDMI, PRIV_OUTPUT_DP, PRIV_OUTPUT_EDP, PRIV_OUTPUT_DSI,
};

// PCH Types
enum pch_info_priv {
	PCH_NONE = 0, PCH_IBX, PCH_CPT, PCH_LPT, PCH_SPT, PCH_CNP, PCH_ICP, PCH_MCC, PCH_TGP, PCH_JSP, PCH_ADP,
};

// --- PCH Device IDs (LPC/ISA bridge) ---
#define INTEL_PCH_DEVICE_ID_MASK		0xff00 // Mask to compare against the TYPE defines below

#define INTEL_PCH_IBX_DEVICE_ID_TYPE	0x3b00 // Ibex Peak (Gen5)
#define INTEL_PCH_CPT_DEVICE_ID_TYPE	0x1c00 // Cougar Point (Gen6)
#define INTEL_PCH_PPT_DEVICE_ID_TYPE	0x1e00 // Panther Point (Gen7 IvyBridge)
#define INTEL_PCH_LPT_DEVICE_ID_TYPE	0x8c00 // Lynx Point (Gen7 Haswell)
#define INTEL_PCH_LPT_LP_DEVICE_ID_TYPE	0x9c00 // Lynx Point LP
#define INTEL_PCH_WPT_DEVICE_ID_TYPE	0x8c80 // Wildcat Point (Gen8 Broadwell) - Note: diff pattern
#define INTEL_PCH_WPT_LP_DEVICE_ID_TYPE	0x9c80 // Wildcat Point LP

#define INTEL_PCH_SPT_DEVICE_ID_TYPE	0xa100 // Sunrise Point (Gen9 Skylake)
#define INTEL_PCH_SPT_LP_DEVICE_ID_TYPE	0x9d00 // Sunrise Point LP
#define INTEL_PCH_KBP_DEVICE_ID_TYPE	0xa200 // Kaby Lake PCH (Union Point / 200-series)
#define INTEL_PCH_GLK_DEVICE_ID_TYPE    0x3100 // Gemini Lake PCH (SoC-like)

#define INTEL_PCH_CNP_DEVICE_ID_TYPE	0xa300 // Cannon Point (Gen10 Cannon Lake)
#define INTEL_PCH_CNP_LP_DEVICE_ID_TYPE	0x9d80 // Cannon Point LP
#define INTEL_PCH_CMP_H_DEVICE_ID_TYPE	0xa380 // Comet Lake H PCH (400-series)
#define INTEL_PCH_CMP_LP_DEVICE_ID_TYPE	0x0280 // Comet Lake LP/U PCH (400-series) - check exact IDs, e.g. 0x02c0
#define INTEL_PCH_CMP_V_DEVICE_ID_TYPE	0xa300 // Comet Lake V PCH

#define INTEL_PCH_ICP_LP_DEVICE_ID_TYPE	0x3400 // Ice Lake LP PCH
#define INTEL_PCH_ICP_N_DEVICE_ID_TYPE	0x3800 // Ice Lake N PCH

#define INTEL_PCH_MCC_DEVICE_ID_TYPE    0x4d00 // Elkhart Lake (Mule Creek Canyon) SoC, not separate PCH for display typically

#define INTEL_PCH_TGP_LP_DEVICE_ID_TYPE	0xa000 // Tiger Lake LP PCH
#define INTEL_PCH_TGP_H_DEVICE_ID_TYPE	0x4300 // Tiger Lake H PCH
#define INTEL_PCH_JSP_N_DEVICE_ID_TYPE	0x4b00 // Jasper Lake N PCH (SoC)

#define INTEL_PCH_ADP_S_DEVICE_ID_TYPE	0x7a00 // Alder Lake S PCH (600-series)
#define INTEL_PCH_ADP_P_DEVICE_ID_TYPE	0x5100 // Alder Lake P PCH
#define INTEL_PCH_ADP_M_DEVICE_ID_TYPE	0x5400 // Alder Lake M PCH (SoC)
#define INTEL_PCH_ADP_N_DEVICE_ID_TYPE	0x5480 // Alder Lake N PCH (SoC)

#define INTEL_PCH_RPL_S_DEVICE_ID_TYPE  0x7900 // Raptor Lake S PCH (700-series) - example, verify

// Simpler defines for matching (using the masked value)
#define INTEL_PCH_IBX_MATCH  (INTEL_PCH_IBX_DEVICE_ID_TYPE & INTEL_PCH_DEVICE_ID_MASK)
#define INTEL_PCH_CPT_MATCH  (INTEL_PCH_CPT_DEVICE_ID_TYPE & INTEL_PCH_DEVICE_ID_MASK)
#define INTEL_PCH_PPT_MATCH  (INTEL_PCH_PPT_DEVICE_ID_TYPE & INTEL_PCH_DEVICE_ID_MASK)
#define INTEL_PCH_LPT_MATCH  (INTEL_PCH_LPT_DEVICE_ID_TYPE & INTEL_PCH_DEVICE_ID_MASK)
#define INTEL_PCH_LPT_LP_MATCH (INTEL_PCH_LPT_LP_DEVICE_ID_TYPE & INTEL_PCH_DEVICE_ID_MASK)

#define INTEL_PCH_SPT_MATCH  (INTEL_PCH_SPT_DEVICE_ID_TYPE & INTEL_PCH_DEVICE_ID_MASK)
#define INTEL_PCH_SPT_LP_MATCH (INTEL_PCH_SPT_LP_DEVICE_ID_TYPE & INTEL_PCH_DEVICE_ID_MASK)
#define INTEL_PCH_KBP_MATCH  (INTEL_PCH_KBP_DEVICE_ID_TYPE & INTEL_PCH_DEVICE_ID_MASK)
#define INTEL_PCH_GLK_MATCH  (INTEL_PCH_GLK_DEVICE_ID_TYPE & INTEL_PCH_DEVICE_ID_MASK)

#define INTEL_PCH_CNP_MATCH  (INTEL_PCH_CNP_DEVICE_ID_TYPE & INTEL_PCH_DEVICE_ID_MASK)
#define INTEL_PCH_CNP_LP_MATCH (INTEL_PCH_CNP_LP_DEVICE_ID_TYPE & INTEL_PCH_DEVICE_ID_MASK)

#define INTEL_PCH_ICP_LP_MATCH (INTEL_PCH_ICP_LP_DEVICE_ID_TYPE & INTEL_PCH_DEVICE_ID_MASK)
#define INTEL_PCH_ICP_N_MATCH (INTEL_PCH_ICP_N_DEVICE_ID_TYPE & INTEL_PCH_DEVICE_ID_MASK)

#define INTEL_PCH_TGP_LP_MATCH (INTEL_PCH_TGP_LP_DEVICE_ID_TYPE & INTEL_PCH_DEVICE_ID_MASK)
#define INTEL_PCH_TGP_H_MATCH (INTEL_PCH_TGP_H_DEVICE_ID_TYPE & INTEL_PCH_DEVICE_ID_MASK)
#define INTEL_PCH_JSP_N_MATCH (INTEL_PCH_JSP_N_DEVICE_ID_TYPE & INTEL_PCH_DEVICE_ID_MASK)

#define INTEL_PCH_ADP_S_MATCH (INTEL_PCH_ADP_S_DEVICE_ID_TYPE & INTEL_PCH_DEVICE_ID_MASK)
#define INTEL_PCH_ADP_P_MATCH (INTEL_PCH_ADP_P_DEVICE_ID_TYPE & INTEL_PCH_DEVICE_ID_MASK)
#define INTEL_PCH_ADP_M_MATCH (INTEL_PCH_ADP_M_DEVICE_ID_TYPE & INTEL_PCH_DEVICE_ID_MASK)
#define INTEL_PCH_ADP_N_MATCH (INTEL_PCH_ADP_N_DEVICE_ID_TYPE & INTEL_PCH_DEVICE_ID_MASK)

#define INTEL_PCH_RPL_S_MATCH (INTEL_PCH_RPL_S_DEVICE_ID_TYPE & INTEL_PCH_DEVICE_ID_MASK)
...
typedef struct {
	...
	uint8_t  panel_bits_per_color;
	bool     panel_is_dual_channel;
	uint8_t  backlight_control_source;
	uint8_t  dpcd_revision;
	uint8_t  dp_max_link_rate;
	uint8_t  dp_max_lane_count;
	bool     dp_tps3_supported;
	bool     dp_enhanced_framing_capable;
	bool     is_pch_port;
	enum pipe_id_priv current_pipe_assignment;
	uint8_t  physical_width_cm;
	uint8_t  physical_height_cm;
} intel_output_port_state;

#define VBT_BACKLIGHT_CPU_PWM 0
#define VBT_BACKLIGHT_PCH_PWM 1
#define VBT_BACKLIGHT_EDP_AUX 2


typedef struct intel_clock_params_t {
	uint32_t pixel_clock_khz;
	uint32_t adjusted_pixel_clock_khz;
	uint32_t cdclk_freq_khz;
	uint32_t lcpll_freq_khz;
	uint32_t lcpll_cdclk_div_sel;
	int      selected_dpll_id;
	bool     is_wrpll;
	uint32_t dpll_vco_khz;
	uint32_t wrpll_n;
	uint32_t wrpll_m2;
	bool     wrpll_m2_frac_en;
	uint32_t wrpll_m2_frac;
	uint32_t wrpll_p1;
	uint32_t wrpll_p2;
	...
	bool     is_lvds;
	bool     is_dp_or_edp;
	uint32_t dp_link_rate_khz;
	uint32_t pixel_multiplier; // For DP, if DisplayID modes need it.
	bool     needs_fdi;
	struct {
		uint16_t tu_size;
		uint16_t data_m;
...
typedef struct intel_i915_device_info {
	...
	uint32_t cursor_gtt_offset_pages[PRIV_MAX_PIPES];
	bool     cursor_visible[PRIV_MAX_PIPES];
	uint16_t cursor_width[PRIV_MAX_PIPES];
	uint16_t cursor_height[PRIV_MAX_PIPES];
	uint16_t cursor_hot_x[PRIV_MAX_PIPES];
	uint16_t cursor_hot_y[PRIV_MAX_PIPES];
	int16_t  cursor_x[PRIV_MAX_PIPES];
	int16_t  cursor_y[PRIV_MAX_PIPES];
	uint32_t cursor_format[PRIV_MAX_PIPES];
