...
#define LP0(offs, rt, name, bt, bn) __attribute__((__gnu_inline__)) \
static __inline__ rt name(void) \
{ \
...
#define LP0NR(offs, name, bt, bn) __attribute__((__gnu_inline__)) \
static __inline__ void name(void) \
{ \
   register struct Library *const _bn __asm("a6") = (struct Library *const)(bn); \
   __asm volatile ("jsr a6@(-"#offs":W)" \
   : \
...
#define LP1(offs, rt, name, t1, v1, r1, bt, bn) __attribute__((__gnu_inline__)) \
static __inline__ rt name(t1 v1) \
{ \
   register rt _re __asm("d0"); \
   register struct Library *const _bn __asm("a6") = (struct Library *const)(bn); \
...
#define LP1NR(offs, name, t1, v1, r1, bt, bn) __attribute__((__gnu_inline__)) \
static __inline__ void name(t1 v1) \
{ \
   register struct Library *const _bn __asm("a6") = (struct Library *const)(bn); \
   register t1 _v1 __asm(#r1) = v1; \
   __asm volatile ("jsr a6@(-"#offs":W)" \
...
void CloseLibrary(struct Library * last);
...
void ClearScreen(struct RastPort * last);
...
void LoadRGB4(struct ViewPort * par1, const uint16 * par2, long last);
void Move(struct RastPort * par1, long par2, long last);
void SetAPen(struct RastPort * par1, unsigned long last);
void SetBPen(struct RastPort * par1, unsigned long last);
void SetDrMd(struct RastPort * par1, unsigned long last);
...
void QueryKeys(struct KeyQuery * par1, unsigned long last);
