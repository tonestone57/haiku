...
alloc_real_mem_callback(struct of_arguments *args)
{
	...
	void* address = find_free_physical_range(length);
	if (address == PHYSINVAL) {
		error = -1;
		return OF_FAILED;
	}

	if (insert_physical_allocated_range((addr_t)address, length) != B_OK) {
		error = -1;
		return OF_FAILED;
	}

	physicalAddress = (intptr_t)address;
	error = 0;
	return B_OK;
}
...
arch_mmu_init(void)
{
	...
	dprintf("MMU: SDR1=0x%08lx, HTAB base=0x%08lx, mask=0x%08lx\n",
