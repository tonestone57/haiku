template<typename T>
bool ThreadPriorityQueue<T>::Add(T thread)
{
    if (_size >= MAX_SIZE)
        return false;
...
template<typename T>
T ThreadPriorityQueue<T>::PopMinimum()
{
    if (_size == 0)
        return nullptr;
...
template<typename T>
bool ThreadPriorityQueue<T>::Remove(T thread)
{
    int index;
...
template<typename T>
bool ThreadPriorityQueue<T>::Update(T thread)
{
    int index;
    if (!_threadMap.Get(thread, index))
        return false;
...
template<typename T>
void ThreadPriorityQueue<T>::HeapifyUp(int i)
{
...
template<typename T>
void ThreadPriorityQueue<T>::HeapifyDown(int i)
{
    while (2 * i <= _size) {
        int j = 2 * i;
...
template<typename T>
void ThreadPriorityQueue<T>::Swap(int i, int j)
{
