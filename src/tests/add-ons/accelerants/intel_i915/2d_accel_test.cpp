...
{
	const char* device_path = "/dev/graphics/intel_i915";
	if (argc > 1)
		device_path = argv[1];

	char* device_paths[16];
	int device_count = 0;
	for (int i = 0; i < 16; i++) {
		char path[256];
		sprintf(path, "%s/%d", device_path, i);
		int fd = open(path, B_READ_WRITE);
		if (fd < 0)
			break;
		device_paths[device_count++] = strdup(path);
		close(fd);
	}

	for (int i = 0; i < device_count; i++) {
		int fd = open(device_paths[i], B_READ_WRITE);
		if (fd < 0) {
